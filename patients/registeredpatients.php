...
<?php include('../includes/sidebar2.php') ?>
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2 text-secondary">Register,<span class="h5">Manage Doctors</span></h1>
      ...
      <div class="table-responsive small">
          <?php
         $sel=mysqli_query($con,"SELECT * FROM province,district,sector,cells,villages,patients,insurances WHERE patients.pat_province=province.province_id and patients.pat_district=district.district_id and patients.pat_sector=sector.sector_id and patients.pat_cell=cells.cell_id and patients.pat_village=villages.village_id and patients.pat_ins=insurances.ins_id");
...
<tbody>
                <tr><form method="post" action="managedoctorlogic.php">
                  <?php $idse=$fec["pat_id"];
                        $quase=$fec["pat_fname"]; 
                  ?>
                  <td hidden><input type="text" name="pat__id" value="<?php echo $fec["pat_id"]?>" class="input-sm" ></td>    
                  <td>
<input type="text" name="pat_fname" value="<?php echo $fec["pat_fname"]?>"class="form-control-sm " >
                  </td>
                  <td><input type="text" name="pat_name" value="<?php echo $fec["pat_lname"]?>"class="form-control-sm " ></td>

                  <td><input type="text" name="nid" value="<?php echo $fec["pat_nid"]?>"class="form-control-sm " ></td>   
                  <td>
               <select class="form-control-sm " name="insurance" required> 
         <?php
        $selectspe=mysqli_query($con,"SELECT * FROM insurances ");
        if (mysqli_num_rows($selectspe)>0) {
          ?>
          <option value="<?php echo $fec["ins_id"]?>"><?php echo $fec["ins_name"]?></option>
          <?php
        while ($frow=mysqli_fetch_array($selectspe)) { 
          ?>
      ...
      <?php
         ...
         ?>
               ...
               <select class="form-control-sm province" name="province" required> 
         <?php

        $selectpro=mysqli_query($con,"SELECT * FROM province ");
        if (mysqli_num_rows($selectpro)>0) {
          ?>
          <option value="<?php echo $fec["pat_province"]?>"><?php echo $fec["province_name"]?></option>
          <?php
      ...
      <?php
         ...
         ?>
                  ...
                  <td>   
    <select name="district"  class="form-control-sm district" required>
        <option  value="<?php echo $fec["pat_district"]?>"><?php echo $fec["district_name"]?></option>
    </select>
              </td>
                  <td>
    <select  name="sector"  class="form-control-sm sector" required>
        <option value="<?php echo $fec["pat_sector"]?>"><?php echo $fec["sector_name"]?></option>
    </select>

                  </td>
                  <td>
    <select name="cell"  class="form-control-sm cell" required> 
        <option value="<?php echo $fec["pat_cell"]?>"><?php echo $fec["cell_name"]?></option>
    </select>
                  </td>
                  <td>
                    <select name="village"  class="form-control-sm village" required>
                     <option value="<?php echo $fec["pat_village"]?>"><?php echo $fec["village_name"]?></option>
                  </select>
                  </td>
                  <td><input type="text" name="pat_contact" value="<?php echo $fec["pat_contact"]?>"class="form-control-sm " ></td>
                  <td><input type="email"   name="email" value="<?php echo $fec["pat_email"]?>" class="form-control-sm " >
                  </td>
                  <td><input type="text" disabled value="<?php echo $fec["created_at"]?>" class="form-control-sm "></td>

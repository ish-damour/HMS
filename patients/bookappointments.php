    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      <!-- <canvas class="my-4 w-100" id="myChart" width="900" height="380"></canvas> -->
              ...
              <div class="container col-md-7  shadow border   p-3 my-3">
                <form role="form" name="book" method="post" action="bookappointmentslogic.php" class="border-radius-6">
      <?php echo $message ?>
...
<!--                <label for="Doctor Specialization">
         Doctor Name
               ...
               <select class="form-control my-2" name="nid" required> 
         <?php

        $sele1=mysqli_query($con,"SELECT * FROM patients ");
        if (mysqli_num_rows($sele1)>0) {
          ?>
          <option value="">Select NID of patient</option>
          <?php
        while ($frow=mysqli_fetch_array($sele1)) { 
          ?>
        <option value="<?php echo $frow["pat_nid"] ?>"><?php echo $frow["pat_nid"] ?></option>
      <?php
        }
        }else{
          ?>
...
<!--                  <label for="Doctor Specialization">
   ...
   <!--             <label for="Doctor Specialization">
        ...
        $selectpro=mysqli_query($con,"SELECT DISTINCT ds.*
        FROM doctor_specialization ds
        JOIN appointments a ON a.appo_specialization_id = ds.Specialization_id 
        and a.start_date>'$daiwe'");
        if (mysqli_num_rows($selectpro)>0) {
          ?>
          <option value=''>Select Specialization needed </option>
          <?php
      ...
      <?php
          ...
          ?>
          <option>no appointment<?php echo $daiwe ?></option>
          <?php
        }
         ?>
         </select>

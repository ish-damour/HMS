...
if(isset($_POST['signin'])){
 ...
 //$rem=$_POST["rememberme"];
  if(!empty($user&&$pass)){
  $que=mysqli_query($con,"SELECT * FROM registration where email='$user' OR username='$user'");
  if(mysqli_num_rows($que)>0){
    ...
    if((($a==$user OR $b==$user) AND $pass==$c)){
      ...
      $_SESSION["loggeduseremail"]=$b; 
      $_SESSION["loggeduserid"]=$fec["reg_id"];   
      echo "<script>alert('Welcome Back register $a')</script>";  
     ?>
